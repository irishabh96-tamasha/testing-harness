...
> **📋 TEMPLATE**: This command uses `main`, `MOB`, and `tamasha-live/{{GITHUB_REPO}}` placeholders. Replace with your project values.
...
gh pr merge <NUMBER> --squash --subject "type(scope): description [MOB-XXX]"
```
...
gh api repos/tamasha-live/{{GITHUB_REPO}}/tags --jq '.[0:5] | .[].name'
```
...
gh api repos/tamasha-live/{{GITHUB_REPO}}/branches --jq '.[].name' | grep -v '^main$\|^dev$'
```

For each stale branch from a merged PR:

```bash
gh api -X DELETE repos/tamasha-live/{{GITHUB_REPO}}/git/refs/heads/<BRANCH_NAME>
```
...
echo "=== Remote ===" && gh api repos/tamasha-live/{{GITHUB_REPO}}/branches --jq '.[].name'
...
| `MOB` | Linear/Jira ticket prefix | `WOR`, `SCA`, `PROJ` |
| `tamasha-live` | GitHub organization | `bybren-llc` |
