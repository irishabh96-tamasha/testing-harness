...
COMMANDS_DIR = Path("/home/ronak/Projects/mobile-app/.claude/commands")
...
def process_workflow_command(filepath: Path):
    ...
    # Replace MOB- with {TICKET_PREFIX}-
    content = content.replace("MOB-", "{TICKET_PREFIX}-")
...
def process_rollback_dev(filepath: Path):
    ...
    patterns = {
        ...
        "ronak@pop-os": "{REMOTE_USER}@{REMOTE_HOST}",
        "pop-os:": "{REMOTE_HOST}:",
        "http://pop-os:": "http://{REMOTE_HOST}:",
        "~/Projects/tamasha-team": "{PROJECT_PATH}",
        "ghcr.io/tamasha-live/tamasha-app/dev": "{REGISTRY}/{PROJECT_NAME}/dev",
        "mobile-app-dev": "{PROJECT}-dev-app",
        "name=tamasha": "name={PROJECT}",
        "Pop OS": "{DEV_MACHINE}",
        "MOB-": "{TICKET_PREFIX}-",
    }
    content = replace_patterns(content, patterns)
    ...
    placeholders = [
        ...
        ("{PROJECT_PATH}", "Project directory on remote", "`~/Projects/tamasha-team`, `~/app`"),
        ("{REGISTRY}", "Container registry URL", "`ghcr.io/tamasha-live`"),
        ("{PROJECT_NAME}", "Project name in registry", "`tamasha-app`, `myapp`"),
        ("{PROJECT}", "Short project identifier", "`tamasha`, `myapp`"),
